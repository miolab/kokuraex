            ...
            |> Enum.map(
              &%{
                ...
                | title: &1["title"],
                  started_at:
                    &1["started_at"]
                    |> DatetimeFunction.datetime_from_iso8601()
                    |> DatetimeFunction.return_datetime(),
                  ended_at:
                    &1["ended_at"]
                    |> DatetimeFunction.datetime_from_iso8601()
                    |> DatetimeFunction.return_datetime(),
                  is_coming_date:
                    &1["ended_at"]
                    |> DatetimeFunction.compare_date_with_current_jst_date()
                    |> UtilFunction.is_eq_or_gt_atom(),
                  catch: &1["catch"],
                  address: &1["address"],
